...
export function updateStatusPane(pane: PaneHandle, model: AppModel): void {
  const target = model.reviewTarget.kind === "working-tree" ? `Working Tree (${model.reviewTarget.scope})` : model.reviewTarget.kind === "commit" ? `Commit ${model.reviewTarget.oid.slice(0, 7)}` : `Stash — ${model.reviewTarget.ref}`
