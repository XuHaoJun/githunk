...
import { createFileTreeState, everyFileInNode, fileTreeRows, someFileInNode, type FileTreeAccessors, type FileTreeMode, type FileTreeRow, type FileTreeState } from "../file-tree"
...
function statusSegments(status: string, nameColor: ColorInput | undefined): readonly ListColumnSegment[] {
  ...
  const colorFor = (char: string, base: ColorInput): ListColumnSegment => (char === " " ? { text: char, ...(nameColor === undefined ? {} : { color: nameColor }) } : { text: char, color: char === "?" ? UNSTAGED_CHANGES_FG : base })
...
function reviewMarkerFor(file: ChangedFile, model: AppModel): string {
  const reviewStatus = model.reviewStatuses !== undefined && Object.prototype.hasOwnProperty.call(model.reviewStatuses, file.path) ? model.reviewStatuses[file.path] : undefined
...
export function filesTreeRows(state: FileTreeState<ChangedFile>, model: AppModel): ListRow[] {
  return fileTreeRows(state).map((row) => {
    ...
    const segments: ListColumnSegment[] = [...(row.indentation.length === 0 ? [] : [{ text: row.indentation }]), ...(row.kind === "directory" ? [named(prefix)] : [...statusSegments(row.status ?? "  ", nameColor), named(" ")]), named(row.name)]
