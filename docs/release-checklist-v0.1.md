...
| Stage/unstage selected changes | Same test applies one parsed diff line through `AppController.applySelection` and verifies cached Git diff | Automated |
| Commit/amend path | Same test commits selected staged content and verifies the commit subject and remaining unstaged content | Automated commit coverage; amend not exercised in this scenario |
...
| Checkout remote branch | Same test checks out `origin/main` through `checkoutRemoteTracking` and verifies local `main` | Automated |
| Pull/fetch/push | Same test calls controller fetch, pull, and push against a real bare remote | Automated |
...
| Left/right and Main/Command Log regions are mouse-resizable | TUI smoke launched the real app, sent vertical and horizontal splitter drag events, and exited cleanly; no geometry readback was available from the non-interactive harness | Manual smoke observed; exact dimensions/readback Not tested |
...
- [x] `bun test tests/acceptance/review-workflow.integration.test.ts` — 1 pass, 47 assertions.
- [x] `bun run typecheck` — run by the controller's final gate.
