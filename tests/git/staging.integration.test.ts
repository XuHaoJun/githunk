...
describe("GitMutations", () => {
  ...
  test("stages only selected additions and leaves the remainder unstaged", async () => {
    ...
    expect(withoutIndex(staged)).toBe(["diff --git a/file.txt b/file.txt", "--- a/file.txt", "+++ b/file.txt", "@@ -1,2 +1,3 @@", " base", "+first", " keep", ""].join("\n"))
    expect(withoutIndex(unstaged)).toBe(["diff --git a/file.txt b/file.txt", "--- a/file.txt", "+++ b/file.txt", "@@ -1,3 +1,4 @@", " base", " first", " keep", "+second", ""].join("\n"))
  })
  ...
  test("stages a selected change in a quoted Unicode path", async () => {
    ...
    expect(withoutIndex(actual)).toBe(['diff --git "a/\\303\\251.txt" "b/\\303\\251.txt"', '--- "a/\\303\\251.txt"', '+++ "b/\\303\\251.txt"', "@@ -1 +1,2 @@", " base", "+changed", ""].join("\n"))
  })
