...
describe("buildPartialPatch", () => {
  test("keeps selected additions and drops unselected additions", () => {
    ...
    expect(transformed(patch, ["+one\n"])).toBe("diff --git a/file.txt b/file.txt\n--- a/file.txt\n+++ b/file.txt\n@@ -1,1 +1,2 @@\n old\n+one\n")
  })
  ...
  test("reverse mode keeps selected additions and makes unselected additions context", () => {
    const patch = "diff --git a/file.txt b/file.txt\n--- a/file.txt\n+++ b/file.txt\n@@ -1,2 +1,3 @@\n old\n+one\n+two\n"
    expect(transformed(patch, ["+one\n"], { reverse: true })).toBe("diff --git a/file.txt b/file.txt\n--- a/file.txt\n+++ b/file.txt\n@@ -1,2 +1,3 @@\n old\n+one\n two\n")
  })
  ...
  test("partial rename strips rename metadata and applies path override", () => {
    ...
    expect(transformed(patch, ["+new\n"], { pathOverride: "new.txt" })).toBe("diff --git a/new.txt b/new.txt\n--- a/new.txt\n+++ b/new.txt\n@@ -1,1 +1,2 @@\n+new\n old\n")
  })
  test("escapes quotes in path headers", () => {
    const patch = "diff --git a/file.txt b/file.txt\n--- a/file.txt\n+++ b/file.txt\n@@ -1,1 +1,1 @@\n-old\n+new\n"
    expect(transformed(patch, ["+new\n"], { pathOverride: 'dir/"quoted".txt' })).toBe('diff --git "a/dir/\\"quoted\\".txt" "b/dir/\\"quoted\\".txt"\n--- "a/dir/\\"quoted\\".txt"\n+++ "b/dir/\\"quoted\\".txt"\n@@ -1,1 +1,2 @@\n+new\n old\n')
  })
