    ...
    test(`${fixture.id}: ${fixture.description}`, () => {
      ...
      for (const expected of fixture.expected.files) {
        ...
        for (const [index, hunk] of expected.hunks.entries()) {
          ...
          const deletionLines = normalized ? normalizedLineSlice(hunkFile.metadata.deletionLines, normalized.deletionLineIndex, hunk.oldCount) : []
          const additionLines = normalized ? normalizedLineSlice(hunkFile.metadata.additionLines, normalized.additionLineIndex, hunk.newCount) : []
      ...
      for (const sample of fixture.expected.rowAddresses) {
        ...
        expect(addresses.some((address) => address.fileKey === sample.fileKey && address.hunkIndex === sample.hunkIndex && ((sample.oldLine !== null && address.side === "old" && address.line === sample.oldLine) || (sample.newLine !== null && address.side === "new" && address.line === sample.newLine)))).toBe(true)
      }
    })
  }
})
