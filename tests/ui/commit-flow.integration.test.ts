...
describe("committing from the files pane", () => {
  ...
  test("c opens a centered summary and description popup from the all scope", async () => {
    ...
    expect((summaryEditor?.screenX ?? 0) + (summaryEditor?.width ?? 0)).toBeLessThanOrEqual((summaryBox?.screenX ?? 0) + (summaryBox?.width ?? 0) - 1)
    expect(hint?.screenY).toBe(descriptionBox?.screenY)
    expect((hint?.screenX ?? 0) + (hint?.width ?? 0)).toBeGreaterThanOrEqual((descriptionBox?.screenX ?? 0) + (descriptionBox?.width ?? 0) - 2)
